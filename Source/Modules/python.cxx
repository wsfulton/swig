...
public:
  ...
  */
  ...
  virtual int functionWrapper(Node *n) {
	...
	Printf(f->code, "  Py_IncRef(swig_obj[i]);\n");
	Printf(f->code, "}\n");
	...
	Printf(f->code, "  Py_IncRef(swig_obj[i + %d]);\n", num_fixed_arguments);
	Printf(f->code, "}\n");
      } else {
	...
	Printf(f->code, "varargs = PyTuple_GetSlice(args, %d, PyTuple_Size(args));\n", num_fixed_arguments);
      ...
      Append(f->code, "Py_DecRef(newargs);\n");
      Append(f->code, "Py_DecRef(varargs);\n");
      Append(f->code, "return resultobj;\n");
      Append(f->code, "}\n");
      Wrapper_print(f, f_wrappers);
    }
  ...
  void builtin_post_decl(File *f, Node *n) {
	...
	Printf(f_init, "Py_DecRef((PyObject *)static_getset);\n");
      } else {
	Printf(getset_def, "    %s,\n", entry);
      }
      Delete(gspair);
    ...
    Printv(f_init, "#endif\n", NIL);
    Printv(f_init, "    }\n", NIL);
    Printv(f_init, "    Py_IncRef((PyObject *)builtin_pytype);\n", NIL);

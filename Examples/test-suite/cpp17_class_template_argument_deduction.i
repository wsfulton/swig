...
// parses and discards it.  The namespace-scope form, the generic form written under a template head
// and the 'explicit' specifier are all accepted.
template <typename T>
struct Bag {
    T value;
    Bag(T v) : value(v) {}
...
template <typename T> Bag(T *) -> Bag<T>;            // generic guide under a template head

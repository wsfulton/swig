...
/* Warning 478 says only that SWIG cannot work the type out, not that there is no type, so
   the python:annotations:mismatch feature supplies it in place of the catch-all type. The
...
%feature("python:annotations:mismatch") argoutBoolTupleThenAppendTyped "typing.List[typing.Union[typing.Tuple[bool, int], int]]"

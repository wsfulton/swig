...
public:
#if 0
   MATLAB():f_begin(0), f_runtime(0), f_header(0), f_doc(0), f_wrappers(0),
  ...
  virtual int importDirective(Node *n) {
    ...
    if (modname)
      Printf(f_init, "feval(\"%s\",matlab_value_list(),0);\n", modname);
  ...
  void dispatchFunction(Node *n) {
    ...
    Printv(f->def, "static matlab_value_list ", wname, " (const matlab_value_list& args, int nargout) {", NIL);
  ...
  virtual int variableWrapper(Node *n) {
    ...
    Printf(setf->def, "static matlab_value_list _wrap_%s(const matlab_value_list& args,int nargout) {", setname);
    ...
    int addfail = 0;
    Printf(getf->def, "static matlab_value_list _wrap_%s(const matlab_value_list& args,int nargout) {", getname);
  ...
  virtual int classHandler(Node *n) {
    ...
    if (use_director) {
      ...
      Printf(disown_shadow, "static matlab_value_list _wrap_disown_%s_shadow " "(const matlab_value_list& args, int nargout) {\n", class_name);
      Printf(disown_shadow, "  if (args.length()!=1) {\n");
      Printf(disown_shadow, "    error(\"disown takes no arguments\");\n");
      Printf(disown_shadow, "    return matlab_value_list();\n");
  ...
  virtual int membervariableHandler(Node *n) {
	...
	NewString("matlab_set_immutable") : Swig_name_wrapper(Swig_name_set(NSPACE_TODO, Swig_name_member(NSPACE_TODO, class_name, symname)));
    assert(s_members_tab);
